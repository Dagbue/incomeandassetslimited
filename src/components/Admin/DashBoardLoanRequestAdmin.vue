    ...
    <div class="body">
      ...
      <div class="row trans-mgt">
        ...
        <div class="form-group fg--search">
          ...
          <input type="text" class="input" placeholder="Search Loan Requests..." v-model="searchQuery" @input="filterDeposits"/>
        </div>
...
export default {
  ...
  computed:{
    ...
    paginatedItems() {
      return this.allLoans.loan.filter(loan =>
    ...
    totalPages() {
      const filteredLoanCount = this.allLoans.loan.filter(loan =>
          Object.values(loan).some(value =>
              value.toString().toLowerCase().includes(this.searchQuery.toLowerCase())
          )
      ).length;
      return Math.ceil(filteredLoanCount / this.itemsPerPage);
    },

    // paginatedItems() {
    //   const startIndex = (this.currentPage - 1) * this.itemsPerPage;
    //   const endIndex = startIndex + this.itemsPerPage;
    //   return this.allLoans.loan.slice(startIndex, endIndex);
    // },
    // totalPages() {
    //   return Math.ceil(this.allLoans.loan.length / this.itemsPerPage);
    // },
    allLoans() {
      return StoreUtils.rootGetters(StoreUtils.getters.loan.getAllLoan)
    },
  },
  ...
  methods: {
    ...
    async approve(child) {
      await StoreUtils.dispatch(StoreUtils.actions.loan.updateLoanStatus, {
        loanId: child.loanId,
        loanStatus: "approved",
      });
      await StoreUtils.dispatch(StoreUtils.actions.loan.readAllLoan)
    },
  ...
  created() {
    ...
    StoreUtils.rootGetters(StoreUtils.getters.loan.getAllLoan)
  },
  ...
  mounted() {
    ...
    StoreUtils.dispatch(StoreUtils.actions.loan.readAllLoan)
  }
}
</script>
